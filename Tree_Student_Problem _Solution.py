...
def check_collision( coordinate_array, radius):
    ...
    is_there_a_collisoin = False

    for c in coordinate_array:
        
        
        collision = tree.closest_point(c)
        
        if abs(c[0] - collision[0]) < radius and abs(c[1] - collision[1]) < radius:
           is_there_a_collisoin = True
            

    return is_there_a_collisoin
